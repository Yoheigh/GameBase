...
public class UIManager : MonoBehaviour
{
    ...
    public UIToast ShowToast(string msg)
    {
        ...
        GameObject go = Managers.Resource.Instantiate($"{name}", pooling: true);        /* 오 이런 문법도 있네 */
    ...
    {
        yield return new WaitForSeconds(1f);        /* 왜 1초? */
        CoCloseToastUI();
    }
    ...
    public void RefreshTimeScale()
    {
        ...
        DOTween.timeScale = 1;          /* 두트윈 타임스케일은 따로 작동하는구나 */
        OnTimeScaleChanged?.Invoke((int)Time.timeScale);        /* 타임 스케일 전환을 감지하는 이벤트도 있네. 근데 뭘 실행시키는 걸까 */
